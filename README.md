...
- **AI 改写可选**：连上 DeepSeek 后可把"嗯那个我觉得吧……"这种口语转写自动洗成书面表达
...
普通语音输入只解决"转成字"，voice-flow 多了一步——把口语转写过一遍 LLM 改写，输出干净的书面表达。

**链路**：

```
按住说话 → ASR (sherpa-onnx) → 改写 (DeepSeek/qwen/...) → 自动粘贴
```

**8 种改写风格**（rewrite profile），覆盖常见写作场景：

| profile  | 用途                              |
| -------- | --------------------------------- |
| `clean`  | 去除口语填充词、修复明显错字       |
| `polish` | 润色为书面语，但保留你的语气       |
| `email`  | 转为正式邮件，加抬头署名段落       |
| `wechat` | 微信口吻，保留口语化但去掉 filler |
| `bullets`| 转 markdown 项目列表               |
| `commit` | 转 Conventional Commit 风格        |
| `prompt` | 优化为给 LLM 的 prompt              |
| `multi`  | 同时输出多个候选版本               |

**Provider 支持**：DeepSeek（默认，便宜 + 中文好）、阿里 DashScope qwen、OpenAI。默认走 DeepSeek，模型为 `deepseek-chat`。

**Fallback**：LLM 调用失败 / 超时 / key 缺失 时自动回退到原始 ASR 文本，不阻塞输入流。

![改写风格选择](./figures/settings-rewrite.png)

> **AI 改写需要 DeepSeek API key**。安装后首次启动会弹出原生输入框让你粘贴；可以跳过（应用仍能正常工作，AI 改写功能关闭）。
