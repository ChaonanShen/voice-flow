...
详细路线见 [docs/plan.md](./docs/plan.md)。其余 ai-coding 执行计划与调试笔记也在 [docs/](./docs) 目录下。本文件只记录从代码本身看不出来的踩坑约束。
...
- 在 **Windows PowerShell** 下读取 `README.md` 以及 `docs/` 下的中文文档时，**必须显式指定 UTF-8**，不要依赖默认编码。
- 标准读法：
  - `Get-Content -Path docs/plan.md -Encoding UTF8`
  - `Get-Content -Path docs/ai_rewrite_plan.md -Encoding UTF8`
  - `Get-Content -Path docs/desktop_gui_plan.md -Encoding UTF8`
  - `Get-Content -Path README.md -Encoding UTF8`
- 如果需要整文件读取，也保持显式编码：`Get-Content -Path docs/plan.md -Encoding UTF8 -Raw`
...
- 主分支必须保持可运行：每个 PR 合并后评委可任意时间检出能跑（见 docs/plan.md §6）。
